...
public class PoseidonStatue extends Block {
    public static final DirectionProperty FACING = Properties.HORIZONTAL_FACING;
    public PoseidonStatue() {
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        ...
        if (GodsData.getRepPoseidon(player) == 15) {
            var stack = player.getStackInHand(hand);
            if (stack.getItem() == Items.TRIDENT) {
                var blocPos = new BlockPos.Mutable(pos.getX(), pos.getY() + 4, pos.getZ());
                world.createExplosion(null, blocPos.getX(), blocPos.getY(), blocPos.getZ(), 2.0F, World.ExplosionSourceType.BLOCK);
                player.setStackInHand(hand, ItemsRegistry.POSEIDON_TRIDENT.getDefaultStack());
            }
        }
